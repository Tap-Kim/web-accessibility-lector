...
├── instructor-rubric.md     # 강사용 채점 루브릭
├── quick-start.md          # 빠른 시작 가이드
└── reports/                # Lighthouse 보고서 및 결과 템플릿
    └── report-template.md
```
...
실습 시에는 `필수 TODO`를 먼저 끝내고, 시간이 남으면 `선택 TODO`를 진행하세요.
1시간 수업 기준 권장 범위는 `Step1~Step3 필수`, `Step4 선택`입니다.

### Step 2 (입문 필수)

| 우선순위 | 작업 | WCAG 2.1 | 빠른 확인법 |
| -------- | ---- | -------- | ----------- |
| 필수 | 스킵 링크 + 메인 랜드마크 연결 | 2.4.1, 1.3.1 | 첫 Tab에서 스킵 링크 노출, Enter 시 메인으로 포커스 이동 |
| 필수 | 시맨틱 구조(`header/nav/main/footer`) 정리 | 1.3.1 | 헤딩/랜드마크 탐색 시 섹션 의미가 읽힘 |
| 필수 | 이미지 `alt`, 폼 `label-for/id` 연결 | 1.1.1, 3.3.2 | 스크린리더가 이미지/입력 목적을 정확히 읽음 |
| 필수 | 버튼/링크 접근 가능한 이름 제공 | 4.1.2 | axe에서 "discernible name" 오류 없음 |
| 선택 | 푸터 내비게이션 레이블 보강 | 2.4.6 | 링크 목적이 목록에서 바로 구분됨 |
| 선택 | 동적 알림용 라이브 리전 추가 | 4.1.3 | 장바구니 변경 시 안내 메시지 낭독 |

### Step 3 (중급 핵심)

| 우선순위 | 작업 | WCAG 2.1 | 빠른 확인법 |
| -------- | ---- | -------- | ----------- |
| 필수 | 검색 토글 `aria-expanded`, 컨트롤 연결 | 4.1.2 | 검색 버튼 상태가 열림/닫힘으로 읽힘 |
| 필수 | 장바구니/상태 변경 Live Region 적용 | 4.1.3 | 수량 변경 시 추가 설명 없이 상태가 낭독됨 |
| 필수 | 가격/평점/배지 정보에 의미 레이블 부여 | 1.3.1 | 의미가 시각 정보 없이도 이해됨 |
| 필수 | 폼 오류(`aria-invalid`, 에러영역) 실시간 안내 | 3.3.1, 3.3.3 | 잘못 입력 시 오류 문구가 즉시 읽힘 |
| 선택 | 단축키(Alt+1/2/3)와 도움말 제공 | 2.1.1 | 단축키로 주요 메뉴 이동 가능 |
| 선택 | 포커스 이동 로직 고도화(오류 필드 우선) | 2.4.3 | 제출 실패 시 첫 오류 필드로 이동 |

### Step 4 (실무 확장)

| 우선순위 | 작업 | WCAG 2.1 | 빠른 확인법 |
| -------- | ---- | -------- | ----------- |
| 필수 | 접근성 도구모음(고대비/큰글씨/포커스) 상태 관리 | 1.4.3, 1.4.4, 2.4.7 | 버튼 `aria-pressed`와 화면 상태가 일치 |
| 필수 | 복합 위젯(검색/제안/필터) 키보드 조작 보장 | 2.1.1, 4.1.2 | Tab/Enter/Esc로 전 기능 조작 가능 |
| 필수 | 재고/긴급/알림 정보의 우선순위 낭독 | 4.1.3 | 품절/재입고 메시지 타이밍이 적절함 |
| 필수 | 상품 카드/위시리스트 상태 식별자 일관성 | 1.3.1, 4.1.2 | 어떤 상품 상태인지 보조기술에서 즉시 파악 |
| 선택 | 개인화 설정(localStorage) 복원 | 3.2.4 | 새로고침 후 접근성 설정 유지 |
| 선택 | 키보드 도움말 모달/치트시트 제공 | 2.4.6 | 단축키 학습 비용 감소 |
...
| Step 3 | 85-95점   | ARIA 속성, Live Regions, 폼 개선   |
| Step 4 | 95-100점  | 완전한 키보드 지원, 고급 위젯      |

환경(브라우저 버전, 확장 프로그램, OS)에 따라 Lighthouse 점수는 달라질 수 있으므로 ±5점 오차는 허용하세요.
